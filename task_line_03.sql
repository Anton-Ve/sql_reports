...
declare
   ...
   l_ENTITY_NM  VARCHAR2(32) := 'value2';
   l_SHEET_NM   VARCHAR2(32) := '';
   l_SHEET_COL  VARCHAR2(32) := '';
   l_SQL_OP     VARCHAR2(7)  := 'CREATE';
   l_ENABLE     number := 1;
   l_EXPORT     number := 0;
   l_LINE_BUILD number := 3;
   l_DESCR      clob := Q'#лицевые счета3#';
   l_SQL_HEADER clob := Q'##';
   l_SQL_QUERY  clob := Q'#select * from ${value2}#'; 
   l_SQL_DATA   clob := Q'# 
   select z.sa_id,z.prem_id from ${value} z
   #';
   l_index number;
begin
